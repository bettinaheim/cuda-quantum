...
namespace Eigen {
  // forward declared so that this header can be used without using Eigen -
  // each library that uses the conversion to eigen will have to link it itself
  template<typename Scalar_, int Rows_, int Cols_, int Options_, int MaxRows_, int MaxCols_>
  class Matrix;
  typedef Matrix<std::complex<double>, -1, -1, 0, -1, -1> MatrixXcd;
}

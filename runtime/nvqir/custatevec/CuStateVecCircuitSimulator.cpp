...
public:
  ...
  cudaq::observe_result observe(const cudaq::spin_op &op) override {
    ...
    const auto cudaqToCustateVec = [](std::string pauli) -> custatevecPauli_t {
      // FIXME: REPLACE THE STRINGS HERE and in other places...
      if (pauli == "I")
    ...
    for (const auto &term : terms) {
      ...
      for (const auto &p : ops) {
        auto p_str = p.to_string(false);
        if (p_str != "I") {
          auto target = p.degrees()[0]; // assumes there is a single target
          paulis.emplace_back(cudaqToCustateVec(p_str));

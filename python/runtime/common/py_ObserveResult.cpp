...
cudaq::spin_op_term to_spin_op_term(py::object &obj) {
  cudaq::spin_op op = py::hasattr(obj, "_to_spinop") ? obj.attr("_to_spinop")().cast<cudaq::spin_op>() : obj.cast<cudaq::spin_op>();
  if (op.num_terms() != 1)
    throw std::invalid_argument("expecting a spin_op with exactly one term");
  return *op.begin();
}
*/
